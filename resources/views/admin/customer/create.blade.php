        ...
        <x-card-container>
            <form action="{{ route('admin.customer.store') }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <x-input type="date" id="date_of_birth" label="Tanggal Lahir" name="date_of_birth" value="{{ old('date_of_birth') }}" required />
                    <x-input id="place_of_birth" label="Tempat Lahir" name="place_of_birth" value="{{ old('place_of_birth') }}" required />
                    <x-input id="identity_number" label="Nomor Identitas" name="identity_number" value="{{ old('identity_number') }}" required />
                    <div>
                        <p>Jenis Kelamin :</p>
                        <div class="mt-2">
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <select id="marrital_status" name="marrital_status" class="block py-3 pl-3 pr-10 w-full text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                <option {{ old('marrital_status') == 'Single' ? 'selected' : '' }} value="Single">Lajang</option>
                                <option {{ old('marrital_status') == 'Married' ? 'selected' : '' }} value="Married">Menikah</option>
                                <option {{ old('marrital_status') == 'Divorved' ? 'selected' : '' }} value="Divorved">Cerai</option>
                            </select>
                        </div>
                    </div>
                    <x-input id="instagram" label="Tautan Instagram" name="instagram" value="{{old('instagram')}}" type="text" required />
                    <x-input id="youtube" label="Tautan YouTube" name="youtube" value="{{old('youtube')}}" type="text" required />
                    <x-input id="facebook" label="Tautan Facebook" name="facebook" value="{{old('facebook')}}" type="text" required />
                    <x-input id="source_of_information" label="Informasi Lainnya" name="source_of_information" value="{{old('source_of_information')}}" type="text" required />
                </div>
                <div class="mt-6">
