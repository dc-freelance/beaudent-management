        ...
        <div>
            ...
            <x-card-container>
                <form action="{{ route('front-office.transaction.payment.confirm', $detailTransaction->id) }}" method="POST">
                    ...
                    <div class="pt-3">
                        <x-input id="transaction_note" label="Catatan" name="transaction_note" />
                    </div>
                    <div class="pt-3">
                    ...
                    <div>
                        ...
                        <div class="pt-3" id="parent_nominal">
                            <x-input id="transaction_additional_discount_nominal" label="Diskon Tambahan :" name="transaction_additional_discount_nominal" type="text" placeholder="Rp." value="Rp. 0" />
                        </div>
                        <div class="pt-3" id="parent_percentage" style="display: none;">
                            <x-input id="transaction_additional_discount_percentage" label="Diskon Tambahan :" name="transaction_additional_discount_percentage" type="number" />
                        </div>
                    </div>
                    <div class="pt-3">
                        <x-input id="transaction_total_ppn" class="bg-gray-300 hover:cursor-not-allowed" label="Total PPN (10%)" name="transaction_total_ppn" type="text" placeholder="Rp." value="Rp. 0" readonly="readonly" required />
                    </div>
                    <div class="pt-3">
                        <x-input id="transaction_grand_total" class="bg-gray-300 hover:cursor-not-allowed" label="Total (Setelah diskon & PPN)" name="transaction_grand_total" type="text" placeholder="Rp." value="Rp. {{ number_format((($detailExaminationTreatment->sum('sub_total')+$detailExaminationItem->sum('sub_total')+$detailAddonExamination->sum('sub_total') - ($totalDiscountTreatment+$totalDiscountItem))), 0, ',', '.') }}" readonly="readonly" required />
                    </div>
                    <div class="pt-3">
                        <x-input id="transaction_total_paid" class="bg-gray-300 hover:cursor-not-allowed" label="Total yang seharusnya dibayarkan (Setelah dipotong deposit)" name="transaction_total_paid" type="text" placeholder="Rp." value="Rp. {{ number_format((($detailExaminationTreatment->sum('sub_total')+$detailExaminationItem->sum('sub_total')+$detailAddonExamination->sum('sub_total') - $detailTransaction->examination->reservation->deposit - ($totalDiscountTreatment+$totalDiscountItem))), 0, ',', '.') }}" readonly="readonly" required />
                    </div>
                    <div class="pt-3">
